...
for filepath in sorted(TESTFILE_DIR.glob("*.xls")):
    if "6月3日" in filepath.name or "6月10日" in filepath.name:
        continue
    ...
    for sheet_name in wb.sheet_names():
        ...
        for r in range(2, ws.nrows):
            ...
            # 统计三明治和酸奶
            if dish and '三明治' in dish["name"]:
                sandwich_count += 1
                if final_sauce and '酸奶' in final_sauce:
                    yogurt_count += 1
                elif final_sauce:
                    pass  # 其他酱料
                else:
                    issues.append(f"  ⚠ Row {int(seq)} {user}: 三明治没有酱料")
            
            # 检查问题
            if not dish:
                issues.append(f"  ✗ Row {int(seq)} {user}: 菜品未匹配 '{dish_name}'")
            elif dish_matched != dish_name and '三明治' in str(dish_matched):
                issues.append(f"  → Row {int(seq)} {user}: '{dish_name}' → '{dish_matched}'")
        ...
        print(f"  三明治总数: {sandwich_count}, 酸奶杯: {yogurt_count}")
        if issues:
